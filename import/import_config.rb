...
module Import
  module Config
    ...
    # Join tables exported as JSON (no ApplicationRecord model). Imported after table dumps.
    STORY_JOIN_TABLE_IMPORTS = [
      { file: 'authors_stories', table: 'authors_stories', columns: %w[user_id story_id] },
      { file: 'stories_story_categories', table: 'stories_story_categories', columns: %w[story_category_id story_id] },
      { file: 'stories_downloads', table: 'stories_downloads', columns: %w[story_download_id story_id] },
      { file: 'stories_csf_instructional_routines', table: 'stories_csf_instructional_routines',
        columns: %w[id story_id csf_instructional_routine_id] },
      { file: 'ece_themes_stories', table: 'ece_themes_stories', columns: %w[story_id ece_theme_id] },
      { file: 'ece_stories_conversation_posters', table: 'ece_stories_conversation_posters',
        columns: %w[story_id ece_conversation_poster_id] },
      { file: 'favorites', table: 'favorites', columns: %w[id story_id illustration_id created_at updated_at] }
    ].freeze
    ...
    def self.table_names_in_bundle(input_dir = INPUT_DIR)
      ...
      dir.children
        ...
        .reject do |name|
          NON_TABLE_JSON.include?(name) ||
            STORY_JOIN_TABLE_IMPORTS.any? { |spec| spec[:file] == name }
        end
    end
